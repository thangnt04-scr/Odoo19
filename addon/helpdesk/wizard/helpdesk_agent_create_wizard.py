...
class HelpdeskAgentCreateWizard(models.TransientModel):
    ...
    email = fields.Char(string="Thư điện tử", required=True)
    ...
    def _check_required_values_not_blank(self):
        for wizard in self:
            if any(
                ...
                raise ValidationError(_("Tên, tài khoản, thư điện tử và mật khẩu không được để trống."))

    def action_create_agent(self):
        self.ensure_one()
        if not self.env.user.has_group("helpdesk.group_helpdesk_supervisor"):
            raise UserError(_("Chỉ giám sát viên mới được tạo nhân viên hỗ trợ."))
